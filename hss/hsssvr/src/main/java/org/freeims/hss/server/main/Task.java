...
import org.freeims.hss.server.cx.op.*;
...
import org.freeims.hss.server.sh.op.PNR;
import org.freeims.hss.server.sh.op.PUR;
import org.freeims.hss.server.sh.op.SNR;
import org.freeims.hss.server.sh.op.UDR;
		...
		if (interface_type == DiameterConstants.Application.Cx){
			...
			switch(command_code){
				case DiameterConstants.Command.MARzh:
					...
					response = org.freeims.hss.server.zh.op.MAR.processRequest(peer, message);
					peer.sendMessage(FQDN, response);
					break;
			}
