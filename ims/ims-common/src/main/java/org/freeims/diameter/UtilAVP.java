...
import org.freeims.diameter.auth.AuthenticationVector;
...
public class UtilAVP {
	...
	private static Logger logger = Logger.getLogger(UtilAVP.class);
	...
	public static int  getResultCode(DiameterMessage message){
		AVP avp = message.findAVP(DiameterConstants.AVPCode.RESULT_CODE, true, 
				DiameterConstants.Vendor.DIAM);
		if (avp != null){
			return avp.getIntData();
		}
	...
	public static String getUserData(DiameterMessage message)
	{
		AVP avp = message.findAVP(DiameterConstants.AVPCode.IMS_USER_DATA, true, 
				DiameterConstants.Vendor.V3GPP);
		if (avp != null){
			return new String(avp.data);
		}
...
//	
	...
	public static void addServerAssignmentType(DiameterMessage message, int typeId){
		AVP avp = new AVP(DiameterConstants.AVPCode.IMS_SERVER_ASSIGNMENT_TYPE, true, DiameterConstants.Vendor.V3GPP);
		avp.setData(typeId);
		message.addAVP(avp);
	}
	public static void addOriginatingHost(DiameterMessage message, String hostName){
		AVP avp = new AVP(DiameterConstants.AVPCode.ORIGIN_HOST, true, DiameterConstants.Vendor.DIAM);
		avp.setData(hostName);
		message.addAVP(avp);
	}
	public static void addOriginatingRealm(DiameterMessage message,String realm){
		AVP avp = new AVP(DiameterConstants.AVPCode.ORIGIN_REALM, true, DiameterConstants.Vendor.DIAM);
		avp.setData(realm);
		message.addAVP(avp);
	}
	
	public static void addOriginatingRequest(DiameterMessage message,int v){
		AVP avp =  new AVP(DiameterConstants.AVPCode.IMS_ORIGINATING_REQUEST, false, 
				DiameterConstants.Vendor.V3GPP);
		avp.setData(v);
		message.addAVP(avp);
	}
	...
	public static void addVisitedNetwork(DiameterMessage message,String vn)
	{
		AVP avp = new AVP(DiameterConstants.AVPCode.IMS_VISITED_NETWORK_IDENTIFIER, 
	...
	public static void addAuthVectors(DiameterMessage message, List avList){
		...
		while (it.hasNext()){
            ...
            if(((av.getAuth_scheme() & CxConstants.Auth_Scheme_Early) != 0)){
            	...
            	logger.info("add DiameterConstants.AVPCode.IMS_SIP_AUTHORIZATION,");
	...
	public static void addUserDataAlreadyAvailable(DiameterMessage message, int id)
	{
		AVP userData = new AVP(DiameterConstants.AVPCode.IMS_USER_DATA_ALREADY_AVAILABLE, true, DiameterConstants.Vendor.V3GPP);
		userData.setData(id);
		message.addAVP(userData);
	}
	
	public static void addUARFlags(DiameterMessage message, int id)
	{
		AVP userData = new AVP(DiameterConstants.AVPCode.IMS_UAR_FLAGS, true, DiameterConstants.Vendor.V3GPP);
		userData.setData(id);
		message.addAVP(userData);
	}
	
	public static void addUserAuthorizationType(DiameterMessage message,int typeID){
		AVP userData = new AVP(DiameterConstants.AVPCode.IMS_USER_AUTHORIZATION_TYPE, true, DiameterConstants.Vendor.V3GPP);
		userData.setData(typeID);
		message.addAVP(userData);
	}
	public static void addShData(DiameterMessage message, String data){
		AVP userData = new AVP(DiameterConstants.AVPCode.IMS_USER_DATA_SH, false, DiameterConstants.Vendor.V3GPP);
		userData.setData(data);
		message.addAVP(userData);
	}
