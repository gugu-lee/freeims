...
package org.freeims.diameterpeer.routing;
...
public class RoutingEngine {
	private static final Logger LOGGER = Logger.getLogger(RoutingEngine.class);
	...
	public Peer getRoute(DiameterMessage msg, PeerManager peerManager) {
		...
		LOGGER.info("application is null:"+(application==null));
		LOGGER.info("destinationRealm is "+destinationRealm);
		if (destinationRealm!=null && destinationRealm.length()>0){
			RoutingRealm rr = realms.get(destinationRealm);
			LOGGER.info("rr is null:"+(rr == null));
			if (rr!=null){
				for (RoutingEntry i : rr.routes) {
					LOGGER.info("i.RQDN:"+i.FQDN);
					
					p = peerManager.getPeerByFQDN(i.FQDN);
					LOGGER.info("p.state:"+p.state);
					if (p!=null && 
