...
import org.freeims.sipproxy.servlet.impl.SipProxyRequestWrapper;
import org.freeims.sipproxy.servlet.impl.SipProxyResponseWrapper;
...
final class ApplicationDispatcher implements AsyncDispatcher, RequestDispatcher {
    ...
    private static class State {
        ...
        /**
         * Outermost SipProxyRequest in the chain
         */
        SipProxyRequest hrequest = null;

        /**
         * Outermost SipProxyResponse in the chain
         */
        SipProxyResponse hresponse = null;
    }
        ...
        throws ServletException, IOException
    {
        ...
        if ((servletPath == null) && (pathInfo == null)) {
            ...
            SipProxyRequestWrapper wrequest =
                (SipProxyRequestWrapper) wrapRequest(state);
            SipProxyRequest hrequest = state.hrequest;
...
//        }
    ...
    private void invoke(ServletRequest request, ServletResponse response,
        ...
        SipProxyResponse hresponse = state.hresponse;
...
//            
//            hresponse.sendError(SipProxyResponse.SC_SERVICE_UNAVAILABLE, sm
...
//                            .getName()));
        ...
        try {
            ...
            if ((servlet != null) && (filterChain != null)) {
            	wrapper.getLogger().info("servlet is created."+servlet.getClass().getName());
    ...
    private ServletRequest wrapRequest(State state) {
        ...
        while (current != null) {
            if(state.hrequest == null && (current instanceof SipProxyRequest))
                state.hrequest = (SipProxyRequest)current;
            if (!(current instanceof ServletRequestWrapper))
                break;
            if (current instanceof SipProxyRequestWrapper)
                break;
            if (current instanceof SipProxyRequest)
                break;
            previous = current;
            current = ((ServletRequestWrapper) current).getRequest();
        }
        ...
        if ((current instanceof SipProxyRequestWrapper) ||
            ...
            (current instanceof SipProxyRequest)) {
            // Compute a crossContext flag
            SipProxyRequest hcurrent = (SipProxyRequest) current;
            boolean crossContext = false;
            if ((state.outerRequest instanceof SipProxyRequestWrapper) ||
                (state.outerRequest instanceof Request) ||
                (state.outerRequest instanceof SipProxyRequest)) {
                SipProxyRequest houterRequest =
                    (SipProxyRequest) state.outerRequest;
...
//                }
            ...
            wrapper = new SipProxyRequestWrapper
                (hcurrent, context, crossContext);
        } else {
            wrapper = new SipProxyRequestWrapper(current);
        }
        if (previous == null)
    ...
    private ServletResponse wrapResponse(State state) {
        ...
        while (current != null) {
            if(state.hresponse == null && (current instanceof SipProxyResponse)) {
                state.hresponse = (SipProxyResponse)current;
                if(!state.including) // Forward only needs hresponse
                    return null;
            }
            if (!(current instanceof ServletResponseWrapper))
                break;
            if (current instanceof SipProxyResponseWrapper)
                break;
            if (current instanceof SipProxyResponse)
                break;
            previous = current;
            current = ((ServletResponseWrapper) current).getResponse();
        }
        ...
        if ((current instanceof SipProxyResponseWrapper) ||
            ...
            (current instanceof SipProxyResponse))
            wrapper =
                new SipProxyResponseWrapper((SipProxyResponse) current,
                        state.including);
        else
            wrapper = new SipProxyResponseWrapper(current, state.including);
        if (previous == null)
    ...
    private void recycleRequestWrapper(State state) {
        ...
        if (state.wrapRequest instanceof SipProxyRequestWrapper) {
            ((SipProxyRequestWrapper) state.wrapRequest).recycle();        
           }
    }
}
